...
import { Particle, hitSpark } from "./particles.js?v=20260604-arcade-upgrade";
...
export class Wingman {
  ...
  updateAttack() {
    ...
    this.game.playerBullets.push(new Bullet(this.x - 5, this.y - 15, -spread, -620, 1, "player", "#69f1ff", 4, false, "playerBullet", 34, 78));
    this.game.playerBullets.push(new Bullet(this.x + 5, this.y - 15, spread, -620, 1, "player", "#69f1ff", 4, false, "playerBullet", 34, 78));
    if (this.level >= 3) {
      this.game.playerBullets.push(new Bullet(this.x, this.y - 20, 0, -680, 2, "player", "#8ffbff", 5, false, "playerBullet", 38, 86));
    }
    this.fireTimer = 0.28 - this.level * 0.035;
  }
  ...
  updateGuard() {
    ...
    if (this.fireTimer <= 0) {
      this.game.playerBullets.push(new Bullet(this.x, this.y - 12, 0, -520, 1, "player", "#fff3a8", 4, false, "playerBullet", 30, 70));
      this.fireTimer = 0.75 - this.level * 0.08;
    }
  }

  updateLaser() {
    if (this.fireTimer > 0) return;
    this.game.playerBullets.push(new Bullet(this.x, this.y - 20, 0, -760, 2 + this.level, "player", "#d05cff", 7, true, "laser", 42, 112));
    this.fireTimer = 0.64 - this.level * 0.07;
  }
