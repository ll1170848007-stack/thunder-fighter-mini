...
import { clamp, distanceSq, rand } from "./utils.js?v=20260604-arcade-upgrade";
...
const TYPES = {
  ...
  bomber: { hp: 4, speed: 74, score: 180, radius: 21, color: "#ff3d3d", fire: 0, sprite: "enemyScout", size: 66, pattern: "bomber", minY: 82, maxY: 190 },
  shield: { hp: 11, speed: 38, score: 360, radius: 28, color: "#7fffd4", fire: 2.7, sprite: "enemyElite", size: 84, pattern: "shield", minY: 88, maxY: 220 },
  laser: { hp: 8, speed: 36, score: 340, radius: 25, color: "#ff315d", fire: 3.0, sprite: "enemyStriker", size: 78, pattern: "sentry", minY: 92, maxY: 210 },
  mineLayer: { hp: 9, speed: 34, score: 350, radius: 25, color: "#d05cff", fire: 2.4, sprite: "enemyStriker", size: 80, pattern: "patrol", minY: 96, maxY: 225 },
  summoner: { hp: 10, speed: 30, score: 420, radius: 27, color: "#8cff5a", fire: 3.2, sprite: "enemyElite", size: 84, pattern: "sentry", minY: 88, maxY: 220 },
  healer: { hp: 9, speed: 32, score: 390, radius: 26, color: "#8cffb0", fire: 2.8, sprite: "enemyElite", size: 82, pattern: "healer", minY: 98, maxY: 230 },
  miniBoss: { hp: 42, speed: 24, score: 1100, radius: 43, color: "#ffb02e", fire: 1.35, sprite: "enemyElite", size: 122, pattern: "miniBoss", minY: 82, maxY: 168 },
};
...
export class Enemy {
  constructor(game, type = "scout", x = rand(40, game.width - 40), y = -40) {
    ...
    this.hp = Math.ceil(spec.hp * (1 + scale * 0.055));
    this.maxHp = this.hp;
  ...
  nextFireDelay() {
    ...
    if (this.type === "miniBoss") return Math.max(0.9, 1.42 - d * 0.04);
    if (this.type === "elite") return 1.85 / (1 + d * 0.035);
    if (this.type === "laser") return 3.1 / (1 + d * 0.03);
    return 2.55 / (1 + d * 0.035);
  }
  ...
  fire() {
    ...
    if (this.type === "mineLayer") {
      this.game.enemyBullets.push(new Bullet(this.x, this.y + this.radius, rand(-24, 24), 44, 1, "enemy", "#d05cff", 8.5, false, null, null, null, { lifeTime: 6 }));
      return;
    }
    if (this.type === "summoner") {
      ...
      this.fireAtAngle(this.angleToPlayer(), 112, "#8cff5a", 4.5);
      return;
    }
    if (this.type === "healer") {
      ...
      this.fireAtAngle(this.angleToPlayer(), 104, "#8cffb0", 4.5);
      return;
    }
    if (this.type === "miniBoss") {
      ...
      for (let i = 0; i < count; i++) {
        ...
        this.fireAtAngle(a, 92, "#ffb02e", 4.8);
      }
      const base = this.angleToPlayer();
      for (const offset of [-0.18, 0, 0.18]) this.fireAtAngle(base + offset, 145, "#ff4b55", 5.2);
      return;
    }
    if (this.type === "sentry") {
    ...
    if (this.type === "bulwark" || this.type === "shield") {
      for (let i = 0; i < 8; i++) this.fireAtAngle((Math.PI * 2 * i) / 8 + this.phase * 0.2, 82, this.type === "shield" ? "#7fffd4" : "#fff3a8", 4.2);
      return;
    }
    ...
    if (this.type === "elite" || this.type === "striker") {
      for (const offset of [-0.45, 0.45]) this.fireAtAngle(angle + offset, 110, "#ffb02e", 4.2);
    }
  }
